...
class networkClass {
    ...
    private networkgo go = null;//async task object
    //String gothtml=""; // for saving the obtained hmtl
    //String goturl=""; //for saving the url if there is anything
    private boolean bolcancel = false;
    private urlandstring pageandurl = new urlandstring("", "");

    class urlandstring //object that takes a url string and an html string
    {
        String url1 = "";
        String html1 = "";
    ...
    private class networkgo extends AsyncTask<Object, String, String> // this will do the network task i assign, since it is a class.. it is an object!
    {
        String html;// url that we will use to get the html crap
        String urll;

        @Override
        protected String doInBackground(Object... perams) {// is not within the UI thread!
    ...
    private Boolean hasend(String s)// checks if there is a com at the end of your url if you don't have http://
    {
        // Log.d("From hasend", end);
        return s.contains(".com") || s.contains(".net") || s.contains(".edu") || s.contains(".gov") || s.contains(".co") || s.contains(".jp") || s.contains(".info");
    }

    void clickload(String url, Handler handler)// with a string we test if the url is properly formatted then call an async task to load a webpage for us
    {
        ...
        if (!url.equals("")) {
            if (hashttp(url) && hasend(url))// if hashttp we open an asyc task and do someting cool!
            {
                ...
                pageandurl.setUrl1(uurl != null ? uurl.toString() : null);
        ...
        } else {//Url handling .. if the url isnt properly formated... does a google search
        }
    }
